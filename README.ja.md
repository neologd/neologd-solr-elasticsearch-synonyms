...
例として以下に「お好み焼き,お好み焼,お好焼」というシノニムファイルのエントリを登録した場合に生成されるペアを示します。

expand 属性の値 | シノニムファイルのエントリ(CSV) | 生成されるペア
--- | --- | ---
...
「expand = true」の場合に、addInternal()は全カラムの文字列の左右が異なるペアを生成します。

「expand = false」の場合には、1カラム目の文字列と 全カラムの各文字列との組み合わせを生成します。
