...
以下に「お好み焼き,お好み焼,お好焼」というシノニムファイルのエントリを登録した場合に生成される対応付けを示します。

expand 属性の値 | シノニムファイルのエントリ(CSV) | 生成される対応付け
--- | --- | ---
...
「expand = true」の場合には全組み合わせが生成されます。

「expand = false」の場合には2カラム目以降の文字列と1カラム目の文字列が対応付けられます。

neologd-solr-elasticsearch-synonyms はおもに「expand = false」で使うことを想定して開発しています。

### 設定ファイルの記述例
Elasticseach と Solr で Solr で解析可能なシノニムファイルを「expand = false」で読み込む際の記述例を示します。
